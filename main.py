...
class ChronosApp:
    ...
    def _render_editor_view(self):
        with ui.row().classes('w-full q-pa-md items-center justify-between bg-[#1e293b] rounded-borders q-mb-md'):
            ...
            with ui.row().classes('q-gutter-sm'):
                ui.button(icon='fullscreen', on_click=lambda: ui.run_javascript('if(window.MKEditor) window.MKEditor.instance.toggleFullScreen()')).props('flat round color=primary').tooltip('Premi ESC per uscire')
